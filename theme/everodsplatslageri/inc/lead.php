 ...
 * or photo. They call ev_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Everöds Plåtslageri AB.
 *
 ...
 * @return string
 */
function ev_sample_note() {
	return ev_lead( 'ar_riktig' ) ? '' : ' <span class="ev-sample">(platshållare)</span>';
}
 ...
 * @return string File name for ev_lead_img().
 */
function ev_lead_bild( $i ) {
	$bilder = (array) ev_lead( 'bilder', array() );
 ...
 * @return string
 */
function ev_ring_label() {
	$namn = ev_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, ev_lead( 'telefon' ) ) : 'Ring ' . ev_lead( 'telefon' );
}

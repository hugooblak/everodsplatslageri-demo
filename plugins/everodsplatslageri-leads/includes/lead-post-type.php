...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'everodsplatslageri-leads' ),
					'singular_name' => __( 'Lead', 'everodsplatslageri-leads' ),
					'edit_item'     => __( 'Förfrågan', 'everodsplatslageri-leads' ),
					'search_items'  => __( 'Search leads', 'everodsplatslageri-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'everodsplatslageri-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'everodsplatslageri-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'everodsplatslageri-leads' ),
			'npl_phone'    => __( 'Phone', 'everodsplatslageri-leads' ),
			'npl_service'  => __( 'Needs', 'everodsplatslageri-leads' ),
			'npl_timeline' => __( 'When', 'everodsplatslageri-leads' ),
			'npl_zip'      => __( 'ZIP', 'everodsplatslageri-leads' ),
			'npl_source'   => __( 'Source', 'everodsplatslageri-leads' ),
			'npl_status'   => __( 'Status', 'everodsplatslageri-leads' ),
			'date'         => __( 'Received', 'everodsplatslageri-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'everodsplatslageri-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'everodsplatslageri-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'everodsplatslageri-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'everodsplatslageri-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'everodsplatslageri-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'everodsplatslageri-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'everodsplatslageri-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'everodsplatslageri-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'everodsplatslageri-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'everodsplatslageri-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'everodsplatslageri-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'everodsplatslageri-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
